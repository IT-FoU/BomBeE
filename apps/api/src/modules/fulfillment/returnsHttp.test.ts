...
describe('returns HTTP', () => {
  ...
  beforeAll(async () => {
    ...
    const identityId = await services.identity.ensureCustomer('+8562097248001', 'Return QA');
  ...
  it('creates and approves a return after delivery', async () => {
    const empty = mockRes();
    await router(mockReq('GET', '/v1/returns'), empty.res);
    expect(empty.res.statusCode).toBe(200);
    expect(empty.body().returns).toEqual([]);

    const none = mockRes();
    await router(mockReq('POST', '/v1/ops/returns/mock-create', {}), none.res);
    expect(none.res.statusCode).toBe(409);
    expect(none.body().error).toBe('no_eligible_delivered_child');
    ...
    await router(mockReq('POST', '/v1/ops/returns/mock-create', { reason: 'defective' }), created.res);
    ...
    expect(created.body().shippingLiability).toBe('store');

    const approve = mockRes();
    await router(mockReq('POST', `/v1/ops/returns/${returnRequestId}/approve`, {}), approve.res);
    expect(approve.res.statusCode).toBe(200);
    expect(approve.body().status).toBe('approved');

    const list = mockRes();
    await router(mockReq('GET', '/v1/returns'), list.res);
    const returns = list.body().returns as Array<{ returnRequestId: string; status: string }>;
    expect(returns.some((r) => r.returnRequestId === returnRequestId && r.status === 'approved')).toBe(
      true,
    );
  });
});
