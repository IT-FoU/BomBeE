...
describe('orders list HTTP', () => {
  ...
  beforeAll(async () => {
    ...
    const identityId = await services.identity.ensureCustomer('+8562097228001', 'Orders QA');
  ...
  it('lists recent parent orders with child summaries', async () => {
    ...
    const list = mockRes();
    await router(mockReq('GET', '/v1/orders?limit=20'), list.res);
    expect(list.res.statusCode).toBe(200);
    const orders = list.body().orders as Array<{
      parentId: string;
      orderNumber: string;
      children: Array<{ status: string }>;
    }>;
    const hit = orders.find((o) => o.parentId === parentId);
    expect(hit?.orderNumber).toBe(orderNumber);
    expect(hit?.children.length).toBeGreaterThan(0);
    expect(hit?.children[0]?.status).toBe('pending_supplier');
  });
});
